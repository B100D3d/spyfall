...
export const encrypt = async (
    ...
    for (const x of range(MESSAGE_WIDTH)) {
        for (const y of range(MESSAGE_HEIGHT)) {
            const sourcePixel = getPixelColor(
                source,
                x + startWidth,
                y + startHeight
            )
            ...
            const encryptPixel =
                messagePixel ^ key.charCodeAt((x + y) % key.length) % 256
...
export const decrypt = async (
    ...
    for (const x of range(380)) {
        for (const y of range(380)) {
            ...
            const secretPixel = getPixelColor(
                secret,
                x + startWidth,
                y + startHeight
            )
            ...
            const solutionPixelColor =
                encryptionNum ^ key.charCodeAt((x + y) % key.length) % 256
